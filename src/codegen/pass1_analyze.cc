...
LOCAL_NODISCARD(Ret codegen_analyze_block(Output& output)) {
    ...
    for (const std::unique_ptr<Adfa>& dfa : dfas) {
        ...
        if (!dfa->cond.empty()) {
            ...
            conds.push_back({dfa->cond, dfa->head->label->index, /*defined*/ true});
        }
    }
    ...
    // If conditions are used, check that all conditions referenced by `=>` and `:=>` exist.
    // In the case of `:=>`, raise an error on non-existent conditions (because the autogenerated
    // semantic action would go to a nonexistent condition start label). In the case of `=>` add
    // the nonexistent condition to condition enum (the user may want to set condition variable
    // and return from the lexer to handle it in user code), but make the initial dispatch abort
    // on it (in case it was a typo).
    if (opts->start_conditions) {
        for (const std::unique_ptr<Adfa>& dfa : dfas) {
            for (const Rule& r : dfa->rules) {
                const char* cond = r.semact->cond;
                if (cond && std::find_if(conds.begin(), conds.end(),
                        [cond](const StartCond& c){ return c.name == cond; }) == conds.end()) {
                    if (r.semact->autogen) {
                        RET_FAIL(output.msg.error(
                                r.semact->loc, "reference to undefined condition '%s'", cond));
                    } else {
                        conds.push_back({cond, output.label_counter++, /*defined*/ false});
                    }
                }
            }
        }
